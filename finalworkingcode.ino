...
// ================== PIN DEFINITIONS ==================
// All pins defined once, in one place
...
#define AMBIENT_LEDS       60
#define KNOCK_LEDS         30
#define MANUAL_LEDS        30

// PWM configuration for fans
#define FAN_PWM_FREQ       25000
#define FAN_PWM_RES        8   // 8-bit => 0..255

// PWM channels (only used for older ESP32 Arduino Core versions)
...
void setup() {
  ...
  knockStrip.setBrightness(120);
  manualStrip.setBrightness(80);

  // Fan PWM setup - Compatible with both old and new ESP32 Arduino Core
  #if ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
    // New API for ESP32 Arduino Core 3.0.0+
    ledcAttach(FAN1_PWM_PIN, FAN_PWM_FREQ, FAN_PWM_RES);
    ledcAttach(FAN2_PWM_PIN, FAN_PWM_FREQ, FAN_PWM_RES);
  #else
    // Old API for older cores
...
void handleCommand(JsonDocument& doc) {
  ...
  if (doc.containsKey("brightness")) {
    ambientBrightness = constrain(doc["brightness"].as<int>(), 0, 255);
    ambientStrip.setBrightness(ambientBrightness);
  ...
  // optional app control for strip 3
  if (doc.containsKey("manual_strip_on")) {
    manualStripEnabled = doc["manual_strip_on"].as<bool>();
    Serial.print("manual_strip_on = ");
    Serial.println(manualStripEnabled ? "true" : "false");
  }
...
void handleKnock(JsonDocument& doc) {
  ...
  int durationMs      = doc["duration_ms"] | 3000;
  ...
  startKnockEffect(String(pattern), durationMs);
}
...
void updateAmbientStrip() {
  ...
  if (currentAmbientMode == MODE_PARTY) {
    ...
    if (currentAmbientMode != lastMode || ambientBrightness != lastBrightness) {
      switch (currentAmbientMode) {
        case MODE_OFF:
          clearStrip(ambientStrip);
          break;
        case MODE_FOCUS:
          setAmbientAll(80, 120, 255);
          break;
        case MODE_PRESENTATION:
          setAmbientAll(255, 180, 80);
          break;
        case MODE_RELAX:
          setAmbientAll(255, 80, 20);
          break;
        case MODE_PARTY:
          break;
      }
...
void updateKnockStrip() {
  ...
  if (knockPattern == "pulse") {
    if (now - knockLastStepMs > 180) {
      ...
      if (knockPhase) fillStrip(knockStrip, 0, 120, 255);
...
void updateManualStrip() {
  ...
  // Direct mapping (NO TOGGLE)
  if (switchState == LOW) {
    manualStripEnabled = true;   // switch active → ON
  } else {
    manualStripEnabled = false;  // switch not active → OFF
  }
